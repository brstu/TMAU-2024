...
<p style="text-align: right;">Ярмолюк Артем Сергеевич</p>
...
В C++ реализовать программу, которая симулирует ПИД-контроллер, обсуждаемый выше. Используйте математическую модель, полученную в предыдущей работе, в качестве объекта управления. Для использования ООП программа должна содержать как минимум 3 класса (+наследование).

Добавьте информацию о разработанной программе (диаграмму классов и т. д.) в отчет с использованием Doxygen (в формате .md).
...
int main() {
    ...
    double linearParamA = 0.9;
    double linearParamB = 0.6;

    // Параметры для нелинейной модели
    double nonLinearParamA = 0.9;
    double nonLinearParamB = 0.6;
    double nonLinearParamC = 0.3;
    double nonLinearParamD = 0.15;
    ...
    PIDControl pidController(1.0, 0.5, 0.25);
